...
--deep = -D 
--changed-use = -U 
 --newuse, -N :: Tells emerge to include installed packages where USE flags have changed since compilation.

--prune, -P ::  Removes  all  but  the  highest  installed version of a package from your system
--unmerge, -C  :: This does no checking of dependencies so deadly 
--emptytree, -e :: Reinstalls target atoms and their entire deep dependency tree, as though no packages are currently installed.
 --complete-graph  [ y | n ] :: consider the deep dependencies of all packages from the world set
 --onlydeps, -o
 --pretend, -p
...
--depclean, -c
 --deselect[=y/n], -W  ::  changes /var/lib/portage/world i.e world set

--getbinpkg = g  :: Download the binary package(s) from a remote binary package host
--getbinpkgonly = -G 
--usepkg = -k     :: binary package(s) in the locally available packages directory
--usepkgonly =  -K
--rebuilt-binaries [ y | n ] :: Replace  installed  packages  with  binary packages that have been rebuilt
